...
def main(args):
    ...
    valid_years = [str(year) for year in np.arange(2006, 2026.1)]
    assert year_str in valid_years
    
    month_str = f"{args.month:02d}"
    valid_months = [str(month) for month in np.arange(1, 12.1)]
    assert month_str in valid_months
    ...
    days = [str(day) for day in np.arange(1, days_in_month + 0.1)]
    ...
    #client = Client()
    client.retrieve(dataset, request, args.outpath)
